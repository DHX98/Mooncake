...
sequenceDiagram
    ...
    loop per 128-key chunk
        ...
        P->>P: classify SSD-only, COMPLETE, size above 0
    end
    alt local holder
...
- **get** (`ssd_get_wait_ms > 0`, default 0 = off): one deadline shared by
  ...
  `kInFlight`/`kCompleted`; re-queries are read-only. The get path never
  initiates promotion and never bypasses the cooldown. On a COMPLETE MEMORY
  replica the transfer plan is rebuilt from the refreshed replica list
